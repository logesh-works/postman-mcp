"""Install bundled slash-command markdown into ``.claude/commands/postman/`` (PRD §C.2b).
...
``init`` re-copies on every run so package upgrades propagate (PRD §C.3).
"""
...
# The six in-Claude-Code commands (PRD §D). init/doctor are terminal-only (§10.2 note).
...
def slash_commands_present(project_root: Path | str = ".") -> bool:
    """Doctor check #5 — all six md files exist under the commands dir (PRD §E)."""
    target = commands_dir(project_root)
