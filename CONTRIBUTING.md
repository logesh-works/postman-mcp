...
Thanks for taking the time to contribute. Postman MCP is an open-source MCP server that
generates and updates Postman requests from API code. This guide gets you from a fresh
...
you agree to uphold it. Report unacceptable behavior to the address listed there.
...
- **Improve docs.** Everything under `docs/` is fair game; small fixes can go straight
  to a PR.
- **Add framework coverage.** New or better parsers under `postman_mcp/input/parsers/`.
- **Write tests.** Coverage gaps are tracked in [ROADMAP.md](ROADMAP.md) and in issues.
...
2. **Keep PRs focused.** One logical change per PR; unrelated cleanups go in their own.
...
6. **Match the surrounding style.** The code favors small, single-responsibility
   modules and comments that explain the why, not the what.
7. Fill out the PR template and link the issue your PR closes.

A maintainer will review. CI must be green before merge.

## Commit messages

Use clear, present-tense summaries, like `Add NestJS guard detection` rather than
`added stuff`. Reference issues with `Fixes #123` where it applies. Conventional Commits
prefixes (`feat:`, `fix:`, `docs:`, `test:`, `refactor:`) are welcome but not required.
