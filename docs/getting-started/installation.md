...
- **Python ≥ 3.10**
- **[Claude Code](https://claude.com/claude-code)** installed. `init` checks for it; if
  it's missing, it prints instructions and still writes config so you can register
  manually.
...
!!! note "Nothing is connected yet"
    `pip install` only puts the bits on disk. All the wiring (the API-key handshake,
